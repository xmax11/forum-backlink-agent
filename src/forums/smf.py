...
class SMFForum(BaseForum):
    """
    Handles login and posting for Simple Machines Forum (SMF).
    """
    ...
    def login(self):
        ...
        if not login_cfg:
            self.logger.warning(f"[SMF] No login credentials for {self.base_url}")
            return session
        ...
        try:
            ...
            self.logger.error(f"[SMF] Failed to load login page: {e}")
            return session

        soup = BeautifulSoup(resp.text, "html.parser")
        form = soup.find("form")
        if not form:
            self.logger.error("[SMF] No login form found")
            return session
        ...
        try:
            ...
            self.logger.info(f"[SMF] Logged in as {username}")
        except Exception as e:
            self.logger.error(f"[SMF] Login failed: {e}")
    ...
    def post_reply(self, session, thread_url, reply_text):
        try:
            ...
            self.logger.error(f"[SMF] Failed to load thread: {e}")
            return False

        soup = BeautifulSoup(resp.text, "html.parser")
        form = soup.find("form")
        if not form:
            self.logger.error("[SMF] No reply form found")
            return False
        ...
        try:
            post_resp = session.post(action_url, data=payload, timeout=15)
            post_resp.raise_for_status()
            self.logger.info(f"[SMF] Posted reply to {thread_url}")
            return True
        except Exception as e:
            self.logger.error(f"[SMF] Failed to post reply: {e}")
            return False
